...
- **Trip detail redesign**: Date range and total distance now appear as translucent accent-color chips directly on the trip map (top-left and bottom-right). The charge-cost card is now a collapsible card with a donut breakdown (distinct shades per stop, AC/DC colored) showing total in the center — tap a segment to see that stop's details; expand to see the per-100km average and the per-charge list, tinted with the matching segment shade. Trip legs are now in a collapsible card too, with a leg-count summary in the header. Drive/Charge leg cards dropped the redundant "Drive N"/"Charge N" label (icon + route/city already conveys it). Under the timeline bar: driving / charging / total durations centered between the start and end clock times. On the timeline itself: a small white icon appears on the first segment of each kind (steering wheel for drives, bolt for DC, plug for AC, P for parking) so the color legend is self-explanatory.
