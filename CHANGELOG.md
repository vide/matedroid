...
- **Drives with incomplete elevation now say so** (#338) — the car only records elevation while it's being polled, so a drive can lose it partway (bad reception, for instance) and end up with elevation for a fraction of the route. Those figures used to be shown as if they covered the whole drive; now the tile, the Elevation card and the profile chart all state which share of the drive they actually cover.
