...
### Added
- **Back up what only your phone knows** — the trips you have merged or renamed yourself, the sentry alerts MateDroid has caught and the way you have the app set up live nowhere else: Teslamate keeps no record of any of it. **Settings → Backup** puts all of it into one file and hands it straight to the share sheet, so it goes wherever you send it — Drive, a chat, a cable — and MateDroid never keeps a copy or decides where it belongs. Every export asks the same short list of questions: sentry alerts and saved place names come ticked, because the place names alone take hours to look up again, while trip maps and everything already downloaded from Teslamate are there if you want them but make the file much bigger. Your API token and server password are never written into the file. If you have more than one car, each one is listed by name and VIN and you tick which of them go in. Putting a backup back works the other way round, through the usual file picker: MateDroid shows you exactly what is inside — which cars, again by name and VIN, and how much of the file belongs to each — before anything changes, and you choose which cars and which parts to take back, and whether to add them to what is already here or replace it. Trips are matched to your drives by when they started rather than by their numbering alone, so restoring onto a rebuilt Teslamate cannot quietly attach a trip to the wrong drive.
