...
### Changed
- **Dependency bump (phase 2a — majors)**: AGP **8.13.2 → 9.1.1**, Gradle **8.14.4 → 9.4.1**, Kotlin **2.2.21 → 2.3.20**, KSP **2.2.21-2.0.5 → 2.3.6** (KSP now decoupled from Kotlin version), Hilt **2.58 → 2.59.2**. AGP 9 ships Kotlin built-in; removed the `org.jetbrains.kotlin.android` plugin alias from the module build scripts. Daemon heap bumped to 6 GB (AGP 9 + Kotlin 2.3 need more RAM during compile). Kotlin 2.3 surfaced a handful of pre-existing issues which were fixed at the source: dead elvis (`?: return`/`?: continue`) on non-null `CarStatus`, redundant `.toFloat()` on results already `Float`, redundant `as ApiResult.Error` after a smart-cast, unnecessary `?.` on a smart-cast non-null `String`, and a broken `DisposableEffect` block in two Fullscreen chart dialogs whose `Context as? Dialog` cast could never succeed (the "make dialog fully fullscreen" imperative was dead code — runtime behavior was already determined by `DialogProperties`, so removing it is a no-op). Detekt 1.23.8 emits one `ReportingExtension.file(String)` Gradle-9 deprecation at configuration time; detekt 2.0-alpha.2 fixes it but requires disabling AGP 9's new DSL — deferred.
