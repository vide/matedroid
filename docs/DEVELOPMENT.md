...
**Which cars.** Both sides tick cars as well as sections. The export picker lists every car the app knows of: `SettingsDataStore.knownCars` — a small id → VIN/name map that `TeslamateRepository.getCars()` refreshes on every successful fetch — unioned with the ids actually present in the local tables, so a car the server has stopped listing does not quietly lose its trips. Persisting that identity is the point: it lets an export made away from a Teslamate that only answers on the home LAN still name its cars and record the VIN a restore matches on. The import dialog lists the cars the *file* holds, with their names and VINs, via `mergeBackupCars(header.cars, ids seen while scanning)` — the header alone is not enough, since it only knows what the exporting phone knew. Trips, sentry alerts and stats follow the ticked cars; place names and trip maps do not, because those caches are keyed by map grid and by trip rather than by car, which is also why Replace clears them whole.
