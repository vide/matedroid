...
/**
 * Draws the shared pin-needle marker base: a thin needle pointing down, a filled circle
 * head in [headColorArgb] with a subtle border. [drawGlyph] optionally draws an icon
 * inside the head circle (centered on cx/headCy, sized relative to headRadius).
 */
private fun createMarkerDrawable(
    resources: Resources,
    headColorArgb: Int,
    drawGlyph: (Canvas, Float, Float, Float) -> Unit = { _, _, _, _ -> }
): Drawable {
    ...
    drawGlyph(canvas, cx, headCy, headRadius)
...
fun createPinMarkerDrawable(resources: Resources, headColorArgb: Int): Drawable =
    createMarkerDrawable(resources, headColorArgb)
...
fun createZapMarkerDrawable(resources: Resources, headColorArgb: Int): Drawable =
    createMarkerDrawable(resources, headColorArgb) { canvas, cx, headCy, headRadius ->
